...
| --- | --- | --- | --- | --- |
| DP-01 | Project creation, reopen, autosave, undo/redo, migration, recovery. | Partial | [`deadpan-store`](../crates/deadpan-store/): durable packages/history, atomic mark transforms and generation relevance, writer ownership, WAL checkpoints, interrupted-attempt recovery, and [schema-1-through-8-to-9 migration tests](../crates/deadpan-store/tests/migration.rs) using old-binary fixtures with requests, attempts, candidate selection, branches and redo. | Managed-media lifecycle, restore/recovery UI, history limits, full failure/chaos suite, and native document workflow. |
| DP-02 | Exact frame/sample/source-time model including VFR. | Partial | Typed rational clocks, VFR intervals and explicit endpoints in core. [`SourceSession`](../crates/deadpan-media/src/source_session.rs) now builds original-PTS indexes from private verified media and performs persistent exact seeks. A strict serialized envelope revalidates core invariants. [Native source evidence](qualification/source-preview-2026-09-21.md) retains measured VFR terminal-duration loss. | Durable media-index integration, complete source policies, and actual shared playback/export, including 10,000 fractional-rate edits. |
...
| DP-15 | Local media import, managed/linked assets, relinking. | Open | Non-destructive source preview reads a private snapshot; it does not register project assets. | Project import, ownership/relink/failure tests. |
...
| DP-19 | Cache integrity and accepted-media portability. | Partial | [Host FFV1 conversion](MEDIA_CONVERSION.md) verifies generated pixels/timing; [generated-object storage](GENERATED_MEDIA.md) owns verified BLAKE3-addressed bytes. [Admission](GENERATION_ACCEPTANCE.md) requires six retained objects before Ready/acceptance and derives assets from measured spans. Relocation, undo/redo/revert and independent real-media readback are exercised. Legacy receipts gain no inferred admission evidence. | Source-clock/color evidence, dependency/history reference tracking, cache eviction, portable copy, and offline-project rendering. |
...
| C: Build the interactive media workspace | Partial | Non-destructive source preview now uses actual persistent decode/index and the shared SDR GPU baseline, with tested frame navigation. Still required: project import, proxies, audio/playback, panes, full keyboard grammar, selection, inspectors, durable history, focus/IME/accessibility corpus and measured editorial latency without drift. |
