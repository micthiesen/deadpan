...
| DP-21 | CLI/JSON API with revision checks and dry-run. | Partial | [Shared headless API](HEADLESS.md): project/command/history operations, explicit migration, indexed picture-plan inspection, exact boundary and named-mark range resolution, original retention/inventory/verification/relinking, source registration/insertion and structured errors. [Registration subprocess tests](../crates/deadpan-cli/tests/source_registration.rs) cover actual media, explicit stream selection, read-only coexistence, stale rejection and required generation context. | Complete command/selector surface, host socket routing, render operations, and headless/GUI parity. |
