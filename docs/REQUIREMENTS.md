...
[host qualification and schema-8 Ready storage](GENERATION_BUNDLES.md).
Explicit durable acceptance remains open.
...
Authored acceptance and application integration remain open.
...
| DP-19 | Cache integrity and accepted-media portability. | Partial | [Host FFV1 conversion](MEDIA_CONVERSION.md) verifies generated RGB pixels/timing in an isolated native helper; [generated-object storage](GENERATED_MEDIA.md) publishes verified BLAKE3-addressed bytes and reads immutable snapshots after relocation. Core generated Hold metadata retains master/provenance references and the original sampling map. [Modern bundle admission](GENERATION_BUNDLES.md) requires three verified output objects before Ready; host qualification also returns three retained input objects bound in provenance. Six-object publication and relocation are integration-tested; legacy receipts remain metadata only. | Qualified acceptance, source-clock/color evidence for retained conditioning, dependency/history reference tracking, cache eviction, portable copy, and offline-project rendering. |
