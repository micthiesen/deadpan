...
strict legacy adapters into database schema 9 and core schema 5. Schemas 7 and 8
already use core schema 5, so their history is validated without rewriting. Existing request
...
All paths retain a `Snapshots/before-schema-9-*.sqlite` backup and promote only the validated
