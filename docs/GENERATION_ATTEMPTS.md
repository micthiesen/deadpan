...
document. Filesystem staging, durable master promotion, explicit acceptance, and
application scheduling remain required work.
...
A receipt records a typed host-managed candidate reference, SHA-256, byte length,
...
Constructing a receipt does not examine any bytes. A future integrated host must
