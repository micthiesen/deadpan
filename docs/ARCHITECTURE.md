...
| `deadpan-models` | Native bridge bundle qualification, retained conditioning bytes, request/provenance binding, and immutable host provenance. | The host derives both masters from one native snapshot and binds pre-launch input snapshots. Ready verifies the three output objects; complete dependency admission and authored acceptance remain gated. |
| `deadpan-media` | Private snapshot conversion and paired native/sampled bridge masters through a bounded helper, exact contracts, and final content identity. | One immutable source and shared hard deadline per pair. Safe host code does not link codecs or mutate projects. |
...
| `deadpan-models` | Pack verification/install, capability planning, AI requests, and candidate validation. | Complete native bridge media/provenance qualification implemented; pack management, audition, qualified acceptance and app integration remain open. |
