...
| `deadpan-audio` | Audio master clock, sample-exact mixing/DSP, tails, and native device output. | Source preparation, room-tone loops, bounded continuous Preserve stages, authored edges, informational meters and finite limiting implemented. The playback/output crates own device integration. Complete voice graph, final mastering and effects remain open. |
