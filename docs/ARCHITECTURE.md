...
| `deadpan-audio` | Exact-phase source resampling, explicit speaker mixing, qualified PCM access, exact room-tone loops and continuous Preserve preparation across nested structural stages. | Preparation workers only; exact affine grids, intrinsic DSP history, transitive source/layout cache provenance and shared work/residency limits. A revision-aware provider supplies media without database coupling. Full voice graph, background cache scheduling and devices remain open. |
...
| `deadpan-audio` | Audio master clock, sample-exact mixing/DSP, tails, and native device output. | Source preparation, room-tone loops and bounded continuous Preserve stages implemented; complete voice graph, clock, effects and device output remain open. |
