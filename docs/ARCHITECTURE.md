...
| `deadpan-audio` | Audio master clock, sample-exact mixing/DSP, tails, and native device output. | Source preparation, room-tone loops, bounded continuous Preserve stages and informational meters implemented; complete voice graph, mastering, clock, effects and device output remain open. |
