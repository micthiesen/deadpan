...
| `deadpan-app` | Native project workspace with `egui`/`eframe`, Metal, a single-writer service, separate import preparation and preview workers. | Create/open, register-only import, explicit whole-source insertion, root-beat Repeat/delete/Hold-duration commands, history, and immutable Source/Sequence frame inspection. Full editing, playback and export remain open. |
...
| `deadpan-store` | Authoritative SQLite document/history, one writer, migrations, recovery, and asset ownership. | SQLite schema 16, schema-1-through-15 migration, writer lock, durable transactions, request relevance, attempts/receipts/selection, interrupted recovery, checkpoints, verified object publication/readback, explicit bundle acceptance, managed/linked originals, relinking and qualified source registration implemented. Full asset lifecycle and application integration remain open. |
