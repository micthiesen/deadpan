...
| `deadpan-plan` | Exact picture mappings and bounded audio spans, sequence duration indexes, compact repeat-run indexes, source-index selection, and deterministic inspection. | Immutable authored revision; no decoder, GPU handle, audio processing, or database connection. Audio retains original coordinates, absolute sample allocation, pitch stages and distinct Hold policies. |
...
| `deadpan-audio` | Exact-phase bounded source resampling, explicit speaker-to-stereo mixing, complete-index-verified original PCM access and plan-driven source-stage blocks. | Preparation workers only; exact affine coordinates and authored trim context. A revision-aware provider supplies media without database coupling. Full voice graph, stretch binding, cache publication and devices remain open. |
...
| `deadpan-audio` | Audio master clock, sample-exact mixing/DSP, tails, and native device output. | Source resampling/matrix, qualified PCM adapter and plan-driven source stage implemented; complete voice graph, clock, effects and device output remain open. |
