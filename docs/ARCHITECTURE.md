...
| `deadpan-source` | Persistent descriptor-only FFmpeg context, metadata scanning, bounded audio inventory, backward seek and owned RGBA conversion. | Bounded cooperative calls, no secondary input opens, explicit SDR interpretation, original source clocks and strict runtime pins. Audio inventory is observed metadata, not decoded sample bounds. |
...
| `deadpan-store` | Authoritative SQLite document/history, one writer, migrations, recovery, and asset ownership. | SQLite schema 10, schema-1-through-9 migration, writer lock, durable transactions, request relevance, attempts/receipts/selection, interrupted recovery, checkpoints, verified object publication/readback, explicit bundle acceptance, managed/linked originals and identity-checked relinking implemented. Full asset lifecycle and application integration remain open. |
