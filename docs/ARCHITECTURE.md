...
| `deadpan-audio` | Exact-phase bounded source resampling, explicit speaker-to-stereo mixing and complete-index-verified original PCM access. | Preparation workers only; exact affine coordinates and authored trim context. Full voice graph, stretch binding, cache publication and devices remain open. |
...
| `deadpan-audio` | Audio master clock, sample-exact mixing/DSP, tails, and native device output. | Source resampling/matrix and qualified PCM adapter implemented; complete voice graph, clock, effects and device output remain open. |
