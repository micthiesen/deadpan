...
| `deadpan-store` | SQLite packages, immutable revisions, atomic edit/history writes, generation requests/attempts, explicit bundle acceptance, writer ownership, checkpoints, and generated-object storage. | SQLite is authoritative. Request relevance changes with authored revisions; attempts remain operational. Acceptance rechecks selection, relevance and all retained objects. |
...
| `deadpan-media` | Verified source snapshots, measured presentation indexes, persistent exact seeks and paired bridge conversion. | Source decoding uses the narrow native adapter on a service thread. Conversion retains its isolated helper and shared hard deadline. Neither path mutates projects. |
| `deadpan-source` | Persistent descriptor-only FFmpeg context, metadata scanning, backward seek and owned RGBA conversion. | Bounded cooperative calls, no secondary input opens, explicit SDR interpretation, original source clocks and strict runtime pins. |
...
| `deadpan-store` | Authoritative SQLite document/history, one writer, migrations, recovery, and asset ownership. | SQLite schema 9, schema-1-through-8 migration, writer lock, durable transactions, request relevance, attempts/receipts/selection, interrupted recovery, checkpoints, BLAKE3 publication/readback, and explicit bundle acceptance implemented. Full asset lifecycle and application integration remain open. |
...
| `deadpan-media` | Qualified FFmpeg/native probing, PTS indexing, bounded decoding, surfaces, encoding/mux interfaces. | Generated RGB-to-FFV1 conversion and persistent H.264/FFV1 source indexes/seeks implemented. Format matrix, durable index cache, project import, playback and export remain open. |
...
| `native/` | Narrow platform and DSP bridges with isolated unsafe lifetime handling. | `deadpan-process` qualifies Darwin zombie-only groups; `deadpan-media-worker` isolates generated-video conversion; `deadpan-source` owns persistent source decoding. Audio/DSP application bridges remain open. |
