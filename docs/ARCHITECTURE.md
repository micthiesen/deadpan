...
| `deadpan-audio` | Exact-phase source resampling, explicit speaker mixing, qualified PCM access, exact room-tone loops, continuous Preserve preparation and informational integrated-loudness/true-peak measurement. | Preparation/analysis workers only; exact affine grids, intrinsic DSP history, transitive source/layout cache provenance and shared work/residency limits. Meters preserve PCM and admit bounded continuous analysis. A revision-aware provider supplies media without database coupling. Full voice graph, mastering, background cache scheduling and devices remain open. |
...
| `deadpan-store` | Authoritative SQLite document/history, one writer, migrations, recovery, and asset ownership. | SQLite schema 20, schema-1-through-19 migration, optional validated single-Original profile and protected full-source baseline, writer lock, durable transactions, request relevance, attempts/receipts/selection, interrupted recovery, checkpoints, verified object publication/readback, explicit bundle acceptance, managed/linked originals, relinking and qualified source registration implemented. Full asset lifecycle and application integration remain open. |
