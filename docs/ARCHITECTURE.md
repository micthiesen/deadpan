...
| `deadpan-plan` | Exact picture mappings, bounded audio spans and occurrence-bound processing stages, sequence duration indexes, compact repeat-run indexes, source-index selection, and deterministic inspection. | Immutable authored revision; no decoder, GPU handle, audio processing, or database connection. Audio separates final sample allocation, explicit grid origin/rule, exact sampling maps and retained envelope progress. Frozen reference plans query old root/Preserve policies; consumption of persisted inserted-time bindings remains open. |
...
| `deadpan-output` | Bounded prepared-PCM queue, generation invalidation, callback reports and a narrow macOS CPAL device boundary. | Headless kernel plus explicit hardware harness; full application clock, lifecycle recovery, rate conversion and release callback qualification remain open. |
| `deadpan-audio` | Exact-phase source resampling, explicit speaker mixing, qualified PCM access, exact room-tone loops, continuous Preserve preparation, sampled-root transfer onto point grids and informational integrated-loudness/true-peak measurement. | Preparation/analysis workers only; exact affine grids, intrinsic DSP history, transitive source/layout cache provenance and shared work/residency limits. Transfer applies old audibility before filtering and retains it at exact destination points, separately from creative fades. Meters preserve PCM and admit bounded continuous analysis. A revision-aware provider supplies media without database coupling. Full voice graph, mastering, background cache scheduling and devices remain open. |
...
receipts and verified originals. Context schema 1 is standalone and explicitly rejects the new nonempty binding state. Binding-aware rendering and atomic Hold insertion remain open.
...
Core 16/database 22 retain bounded clock and symbolic-phase intent with explicit Repeat birth/survivor scope. Rendering rejects nonempty binding state until both policy and PCM consumers honor it. The complete lifecycle remains open.
...
| `deadpan-store` | Authoritative SQLite document/history, one writer, migrations, recovery, and asset ownership. | SQLite schema 22, schema-1-through-21 migration, optional validated single-Original profile and protected full-source baseline, writer lock, durable transactions, request relevance, attempts/receipts/selection, interrupted recovery, checkpoints, verified object publication/readback, explicit bundle acceptance, managed/linked originals, relinking and qualified source registration implemented. Full asset lifecycle and application integration remain open. |
...
| `deadpan-audio` | Audio master clock, sample-exact mixing/DSP, tails, and native device output. | Source preparation, room-tone loops, bounded continuous Preserve stages, authored edge fades and informational meters implemented; complete voice graph, mastering, clock, effects and device output remain open. |
