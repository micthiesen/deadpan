...
| `deadpan-store` | SQLite packages, immutable revision snapshots, atomic edit/history writes, monotonic generation requests, persistent attempts and recovery, undo/redo, writer ownership, and backup API checkpoints. | SQLite is authoritative; exported JSON is inspection only. Request relevance changes atomically with authored revisions; attempts remain operational state. |
...
| `deadpan-store` | Authoritative SQLite document/history, one writer, migrations, recovery, and asset ownership. | SQLite schema 6, complete schema-1/2/3/4/5 migration, writer lock, durable transactions, generation request relevance, attempts/receipts/selection, interrupted-attempt recovery, and checkpoints implemented; full asset lifecycle open. |
