...
J,H=kernels(RADIUS,OVERSAMPLE,'bh4')
...
for name,xx in cases.items():
 ...
 assert q<=10**(-1/20),item
 results.append(item)
print(json.dumps({'numpy':np.__version__,'seconds':time.monotonic()-start,'kernel_l1':float(np.max(np.sum(abs(H),axis=1))),'kernel_distance_sum':float(np.max(np.sum(abs(H)*abs(J),axis=1))),'worst_margin_for_input16':LIMIT-16*np.max(np.sum(abs(H)*abs(J),axis=1))/ATTACK,'results':results},indent=2))
