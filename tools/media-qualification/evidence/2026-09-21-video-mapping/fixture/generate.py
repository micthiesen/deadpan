"""Produce genuine schema-11 SQL using archived, pinned pre-video-mapping Rust."""
...
REPO = HERE.parents[4]
REVISION = '8a6c264e447615c585a2b9f849edfe44f51504fd'
SEED = REPO / 'crates/deadpan-store/tests/fixtures/v10-history.sql'
EXPECTED = REPO / 'crates/deadpan-store/tests/fixtures/v11-history.sql'
HEADER = '''-- Genuine schema-11 project generated and validated with commit
...
scratch = Path(tempfile.mkdtemp(prefix='deadpan-schema11-reproduce-', dir='/tmp'))
...
example = source / 'crates/deadpan-store/examples/schema11_fixture.rs'
example.parent.mkdir(exist_ok=True)
shutil.copyfile(HERE / 'schema11_fixture.rs', example)
env = os.environ.copy()
env['CARGO_TARGET_DIR'] = '/tmp/deadpan-video-mapping-migration/old-target'
...
assert (doctor['document_schema'], doctor['database_schema']) == (6, 11)
...
run(['cargo', 'run', '--manifest-path', source / 'Cargo.toml', '-p', 'deadpan-store',
     '--example', 'schema11_fixture', '--locked', '--', package], env, source)
validation = run([cli, 'project', 'validate', package])
(HERE / 'old-binary-validation.json').write_text(validation)
connection = sqlite3.connect(package / 'project.sqlite')
snapshot = sqlite3.connect(scratch / 'schema11-before-migration.sqlite')
connection.backup(snapshot)
output = HEADER + '\n'.join(snapshot.iterdump()) + '\n'
reproduced = scratch / 'v11-history-reproduced.sql'
...
manifest = {
    ...
    'harness_sha256': digest(HERE / 'schema11_fixture.rs'),
