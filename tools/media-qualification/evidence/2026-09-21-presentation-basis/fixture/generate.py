"""Reproduce schema 14 twice with the pinned pre-basis CLI and store API."""
...
REPO = HERE.parents[4]
REVISION = '14c75218b695e732773a4b247a45db3b7290faac'
SEED = REPO / 'crates/deadpan-store/tests/fixtures/v13-history.sql'
EXPECTED = REPO / 'crates/deadpan-store/tests/fixtures/v14-history.sql'
HEADER = '''-- Genuine schema-14 project generated and validated with commit
-- 14c75218b695e732773a4b247a45db3b7290faac (core schema 9).
-- Starts from v13-history.sql, migrated by that revision's rebuilt CLI.
-- The old store API retains and decodes cfr-bframes.mp4 and vfr.mp4, then
-- atomically registers/inserts both with one reused alias across undo/branch.
-- Both immutable qualifications and original ownership rows survive; rename
-- undo/redo ends with a pending redo. Other operational rows remain unchanged.
-- Generated twice by tools/media-qualification/evidence/
-- 2026-09-21-presentation-basis/fixture/generate.py, byte-for-byte equal.
...
scratch = Path(tempfile.mkdtemp(prefix='deadpan-schema14-reproduce-', dir='/tmp'))
...
example = source / 'crates/deadpan-store/examples/schema14_fixture.rs'
example.parent.mkdir(exist_ok=True)
shutil.copyfile(HERE / 'schema14_fixture.rs', example)
env = os.environ.copy()
env['CARGO_TARGET_DIR'] = '/tmp/deadpan-presentation-basis-migration/old-target'
...
assert (doctor['document_schema'], doctor['database_schema']) == (9, 14)
...
for number in range(2):
    ...
    run(['cargo', 'run', '--manifest-path', source / 'Cargo.toml', '-p',
         'deadpan-store', '--example', 'schema14_fixture', '--locked', '--',
         package], env, source)
    validation = run([cli, 'project', 'validate', package])
    (HERE / f'old-binary-validation-{number}.json').write_text(validation)
    with sqlite3.connect(package / 'project.sqlite') as connection:
        with sqlite3.connect(scratch / f'schema14-backup-{number}.sqlite') as snapshot:
            connection.backup(snapshot)
            output = HEADER + '\n'.join(snapshot.iterdump()) + '\n'
            reproduced = scratch / f'v14-history-reproduced-{number}.sql'
...
manifest = {
    ...
    'harness_sha256': digest(HERE / 'schema14_fixture.rs'),
