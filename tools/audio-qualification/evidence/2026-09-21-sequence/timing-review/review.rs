...
#[test]
fn review_half_sample_ties_skip_zero_allocations_without_resetting_phase() {
    let rate = FrameRate::new(96_000, 1).unwrap();
    let renderer = renderer(rate, &["zero", "half", "integer"], [
        ("zero", source(rate, 1, 0..1, 0)),
        ("half", source(rate, 1, 0..1, 0)),
        ("integer", source(rate, 1, 0..1, 0)),
    ]);
    let mut provider = FixtureProvider::new(1);
    let expected_fractional = reference(
        ResampleRecipe::new(0..1, ratio(-1, 2), AudioSample(0), ExactRatio::ONE, samples(0, 1)).unwrap(),
        0, 1,
    );
    assert_eq!(renderer.plan().audio_duration().unwrap(), AudioSample(2));
    assert_eq!(read(&renderer, &mut provider, 0, 2), [expected_fractional[0], fixture_sample(0)]);
    assert_eq!(provider.calls, 2);
    assert_eq!(read(&renderer, &mut provider, 1, 1), [fixture_sample(0)]);
}
#[test]
fn review_signed_fractional_placement_composes_with_follow_speed_and_crop() {
    let rate = FrameRate::new(48_000, 1).unwrap();
    let mut node = source(rate, 100, 512..612, 0);
    let NodeKind::Source { source } = &mut node.kind else { unreachable!() };
    source.audio_mapping = SourceAudioMapping::Placement { start: ratio(-1, 3), frames: ExactRatio::integer(100) };
    let renderer = renderer(rate, &["crop"], [
        ("source", node),
        ("crop", retime("source", 20, 10..70, PitchPolicy::FollowSpeed)),
    ]);
    let mut provider = FixtureProvider::new(60);
    let recipe = ResampleRecipe::new(523..583, ratio(1567, 3), AudioSample(0), ratio(3, 1), samples(0, 20)).unwrap();
    let expected = reference(recipe, 0, 20);
    assert_eq!(read(&renderer, &mut provider, 0, 20), expected);
    assert_eq!(read(&renderer, &mut provider, 7, 13), expected[7..]);
}
#[test]
fn review_maximum_source_coordinates_remain_exact_at_clipped_edges() {
    let rate = FrameRate::new(48_000, 1).unwrap();
    let end = i64::MAX;
    let renderer = renderer_with_asset(rate, &["source"], [
        ("source", source(rate, 8, end - 8..end, 0)),
    ], audio(end - 8, end).span);
    let query = renderer.plan().audio(samples(0, 8), deadpan_plan::AudioQueryLimits::default()).unwrap();
    let leaf = &query.spans[0];
    assert_eq!(leaf.source_point_at_project_frame(ratio(1, 3)).unwrap().ticks, ExactRatio::integer(end - 8).checked_add(ratio(1, 3)).unwrap());
    assert_eq!(leaf.source_point_at_project_frame(ExactRatio::integer(8)).unwrap().ticks, ExactRatio::integer(end));
    assert_eq!(leaf.source_point(AudioSample(7)).unwrap().ticks, ExactRatio::integer(end - 1));
}
#[test]
fn review_fractional_original_trim_rejects_before_reading_selected_pcm() {
    let rate = FrameRate::new(48_000, 1).unwrap();
    let clock = SourceTimeBase::new(1, 96_000).unwrap();
    let selected = SourceSpan::new(SourceTimestamp { ticks: 1, time_base: clock }, SourceTimestamp { ticks: 5, time_base: clock }).unwrap();
    let mut node = source(rate, 2, 1..5, 0);
    let NodeKind::Source { source } = &mut node.kind else { unreachable!() };
    source.audio = Some(SourceAudio { asset: AssetId::new("media").unwrap(), span: selected });
    source.audio_mapping = SourceAudioMapping::natural_rate(selected, rate).unwrap();
    let renderer = renderer_with_asset(rate, &["source"], [("source", node)], selected);
    let mut provider = FixtureProvider::new(1);
    assert!(matches!(renderer.read_sources(&mut provider, AudioSample(0), 2, TIMEOUT, &AtomicBool::new(false)), Err(SequenceAudioError::Preparation(PreparationError::InvalidRecipe(_)))));
}
