//! Frozen schema-17 document and history adapter. Existing InsertTime commands
//! and audio bindings are retained; schema-18 framing is not admitted.
...
impl Document {
    pub fn from_json(json: &str) -> Result<Self, DocumentError> {
        ...
        if old.schema_version != 17 {
            return Err(invalid("migration requires document schema 17"));
        }
        old.clone().upgrade()?;
        Ok(old)
    }
...
pub fn matches_edit(json: &str, edit: &EditTransaction) -> Result<bool, DocumentError> {
    ...
    // Schema 17 used the same changed-ID summary as current old commands,
