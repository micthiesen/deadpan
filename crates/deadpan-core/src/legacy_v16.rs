//! Frozen schema-16 document and history adapter. Existing audio bindings are
//! retained exactly; new schema-17 commands are deliberately not admitted.
...
impl Document {
    pub fn from_json(json: &str) -> Result<Self, DocumentError> {
        ...
        if old.schema_version != 16 {
            return Err(invalid("migration requires document schema 16"));
        }
        old.clone().upgrade()?;
        Ok(old)
    }
...
pub fn matches_edit(json: &str, edit: &EditTransaction) -> Result<bool, DocumentError> {
    ...
    // Schema 16 used the same changed-ID summary as current old commands,
